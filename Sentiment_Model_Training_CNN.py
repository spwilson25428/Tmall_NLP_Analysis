...
from keras.layers import Dense, Conv1D, GlobalMaxPooling1D, Input, Embedding, concatenate, Dropout
...
maxLength = 24  # we will only cover sentence up to 24 words

# Keras input tokenizer
totalX = [" ".join(wordslist) for wordslist in totalX]
input_tokenizer = Tokenizer(num_words=50000)
...
# Build the model
embedding_size = 128
text_input = Input(shape=(maxLength,), dtype='int32')
input_embed = Embedding(input_dim=vocab_size, output_dim=embedding_size, input_length=maxLength)(text_input)
bigram = Conv1D(filters=128, kernel_size=2, activation='relu')(input_embed)
bigram = GlobalMaxPooling1D()(bigram)
trigram = Conv1D(filters=128, kernel_size=3, activation='relu')(input_embed)
trigram = GlobalMaxPooling1D()(trigram)
fourgram = Conv1D(filters=128, kernel_size=4, activation='relu')(input_embed)
...
output = Dense(units=output_dimen, activation='softmax')(merged)
...
# Plot Model Accuracy
# fig, ax = plt.subplots()
# plt.plot(np.arange(1, 7), history.history['acc'])
# plt.plot(np.arange(1, 7), history.history['val_acc'])
